...
if(!$product) {
    if($is_ajax) { header('Content-Type: application/json'); echo json_encode(array('status'=>'error','message'=>'Ürün bulunamadı')); exit; }
    header('location: index.php');
    exit;
}
...
if($p_stock < 1) {
    if($is_ajax) { header('Content-Type: application/json'); echo json_encode(array('status'=>'error','message'=>'Stokta yok')); exit; }
    header('location: index.php');
    exit;
}
...
if($already) {
    if($is_ajax) { header('Content-Type: application/json'); echo json_encode(array('status'=>'already','message'=>'Bu ürün zaten sepetinizde')); exit; }
...
if($is_ajax) {
    ...
    echo json_encode(array('status'=>'success','message'=>'Sepetinize Eklendi','cart_count'=>$cart_count,'cart_total'=>number_format($cart_total, 2, '.', '')));
    exit;
}
